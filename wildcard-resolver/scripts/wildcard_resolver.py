...
def _install_override_patch():
    ...
    # Cache the tracer's RECORDER once at install time by scanning the closure
    # of _orig (which is the tracer's patched function if tracer loaded first).
    # This avoids repeated closure scans on every single wildcard call.
    _cached_tracer_recorder = None
    try:
        fn = _orig_get_wildcard_random
        for src in [fn, getattr(fn, '__func__', None)]:
            if src and getattr(src, '__closure__', None):
                for cell in src.__closure__:
                    try:
                        obj = cell.cell_contents
                        if hasattr(obj, 'record') and hasattr(obj, '_records'):
                            _cached_tracer_recorder = obj
                            break
                    except ValueError:
                        pass
            if _cached_tracer_recorder:
                break
        if _cached_tracer_recorder:
            print("[WildcardResolver] Tracer RECORDER cached at patch-install time")
        else:
            print("[WildcardResolver] Tracer not found at patch time (tracer not installed, or loaded after resolver)")
    except Exception as _e:
        print(f"[WildcardResolver] Tracer cache error: {_e}")
    ...
    def overriding_get_wildcard(self_s, command, context):
        ...
        if wc_name in overrides:
            ...
            tracer_recorder = _cached_tracer_recorder

            rec_count_before = (
                len(tracer_recorder._records)
                if tracer_recorder and tracer_recorder._records is not None
                else None
            )
            try:
                # Consume exactly ONE yield from the upstream generator.
                # Both the tracer's patch and the real original use `while True`
                # so we must NOT iterate to exhaustion — just advance once to
                # trigger recorder.record() and sub-wildcard resolution.
                upstream_gen = _orig_get_wildcard_random(self_s, command, context)
                next(upstream_gen)
            except (StopIteration, Exception):
                pass

            # Fix tracer records: ensure a top-level record exists for this
            # wildcard with the locked value, then delete any sub-records from
            # the random resolution (they'll be re-added by sample_prompts below).
            if tracer_recorder is not None and tracer_recorder._records is not None and rec_count_before is not None:
                if len(tracer_recorder._records) > rec_count_before:
                    # Upstream added at least one record — overwrite the top-level
                    # and truncate any sub-records from the random pick.
                    tracer_recorder._records[rec_count_before] = (wildcard_path, raw_value)
                    del tracer_recorder._records[rec_count_before + 1:]
                else:
                    # Upstream added NO record (e.g. wildcard file has 0 entries,
                    # fell through to real original which doesn't call recorder).
                    # Manually inject the top-level record so the record tree stays
                    # consistent and _count_records_for can walk it correctly.
                    tracer_recorder._records.append((wildcard_path, raw_value))
...
class WildcardResolverScript(scripts.Script):
    ...
    def before_process(self, p, *args):
        ...
        if CHAIN_START not in prompt:
            # No locks active — clear any overrides left from a previous generation
            # before sd-dynamic-prompts runs its own before_process/process hooks.
            _clear_overrides()
            return
    ...
    def _apply_sentinel(self, p):
        ...
        if cs == -1 or ce == -1:
            # No sentinel present.  Do NOT clear overrides here — if before_process
            # already extracted the sentinel and set overrides, calling
            # _clear_overrides() now would wipe them before sd-dynamic-prompts has
            # had a chance to use them (our process() fallback hook can fire before
            # sd-dynamic-prompts' process() hook depending on extension load order).
            # Overrides are cleared exclusively in postprocess().
            return
